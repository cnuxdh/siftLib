...
void restore_minpq_order( Pq_Node*, int, int );
void decrease_pq_node_key( Pq_Node*, int, int );
...
Min_Pq* minpq_init()
{
	...
	min_pq = new Min_Pq [1];
	min_pq->pq_array = new Pq_Node [MINPQ_INIT_NALLOCD];
...
*/
int minpq_insert( Min_Pq* min_pq, void* data, int key )
{
	int n = min_pq->n;
...
*/
void* minpq_get_min( Min_Pq* min_pq )
{
	if( min_pq->n < 1 )
	{
...
if \a min_pq is empty
*/
void* minpq_extract_min( Min_Pq* min_pq )
{
	void* data;
...
*/
void minpq_release( Min_Pq** min_pq )
{
	if( ! min_pq )
	{
